...
export function ImageGallery({ visible, webContentsId, pageUrl, onClose }: ImageGalleryProps) {
  ...
  useEffect(() => {
    ...
    window.electronAPI.scanPageImages(webContentsId).then((result: DetectedImage[]) => {
      setImages(result || []);
      setLoading(false);
    }).catch(() => {
      setLoading(false);
  ...
  const handleDownloadSelected = useCallback(async () => {
    ...
    const batch = images
      ...
      .map((img) => ({ url: img.url, filename: undefined }));
  ...
  const handleDownloadSingle = useCallback(async (url: string) => {
    let referer: string | undefined;
    try {
      referer = new URL(pageUrl).origin;
    } catch {}
          ...
          <>
            <div className="image-gallery-toolbar">
              ...
              <div className="image-gallery-toolbar-actions">
                ...
                <button
                  onClick={deselectAll}
                  className="image-gallery-btn-text"
                  ...
                  <div className="image-gallery-thumb">
                    ...
                    <div className="image-gallery-checkbox">
                      {selected.has(img.url) ? (
                        <svg width="20" height="20" viewBox="0 0 20 20">
                          <rect x="1" y="1" width="18" height="18" rx="4" fill="var(--accent-color)" stroke="var(--accent-color)" strokeWidth="1.5" />
                          <path d="M5 10l3 3 7-7" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                      ) : (
                        <svg width="20" height="20" viewBox="0 0 20 20">
                          <rect x="1" y="1" width="18" height="18" rx="4" fill="none" stroke="var(--text-tertiary)" strokeWidth="1.5" />
                        </svg>
                      )}
                    </div>
                    <button
                    ...
                    >
                      <svg width="14" height="14" viewBox="0 0 16 16">
                        <path d="M8 1v9M4 7l4 4 4-4M2 13h12" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </button>
                  </div>
                  <div className="image-gallery-info">
                    {(img.naturalWidth > 0 && img.naturalHeight > 0) && (
                      <span className="image-gallery-size">{img.naturalWidth}x{img.naturalHeight}</span>
                    )}
                  </div>
                </div>
              ))}
