...
export function NavigationBar({
  ...
  // Sync URL from active tab during render (React-recommended pattern)
  const prevUrlRef = useRef('');
  const activeUrl = activeTab?.url ?? '';
  if (!isFocused && activeUrl !== prevUrlRef.current) {
  ...
  const handleNavigate = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      if (urlInput.trim()) {
        window.electronAPI.navigate(urlInput.trim());
  ...
  const showDownloadButton = mediaState === 'detected' || mediaState === 'detecting';
  ...
  return (
    <div className="nav-bar">
      <div className="nav-buttons">
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16">
            <path d="M5 1l7 7-7 7" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>
      ...
      <div className="nav-actions">
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16">
            ...
            <path d="M1.5 11l3.5-3.5 2.5 2.5 2.5-3.5L14.5 11" stroke="currentColor" strokeWidth="1.3" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </button>

        {/* Download button: shows when media detected or detecting */}
        {showDownloadButton && (
          <button
            className={`nav-btn ${mediaState === 'detected' ? 'nav-btn-download-active' : 'nav-btn-download-detecting'}`}
            onClick={onDownloadClick}
            title={mediaState === 'detecting'
              ? 'Detecting media...'
