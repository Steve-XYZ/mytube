...
const electronAPI = {
  ...
  pauseDownload: (downloadId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_PAUSE, downloadId),
  resumeDownload: (downloadId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.DOWNLOAD_RESUME, downloadId),
  ...
  scanPageImages: (tabWebContentsId: number) =>
    ipcRenderer.invoke('media:scan-images', tabWebContentsId),
  downloadImage: (url: string, referer?: string) =>
    ipcRenderer.invoke('media:download-image', url, referer),
  downloadImagesBatch: (
  ...
  onActiveTabChanged: (callback: (tabId: string) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC_CHANNELS.TAB_ACTIVE_CHANGED, handler); };
  },
  ...
  onMediaDetected: (callback: (data: unknown) => void) => {
    ...
    return () => { ipcRenderer.removeListener(IPC_CHANNELS.MEDIA_DETECTED, handler); };
  },
  ...
  onFocusUrl: (callback: () => void) => {
    ...
    return () => { ipcRenderer.removeListener('shortcut:focus-url', handler); };
  },
  onToggleFind: (callback: () => void) => {
    const handler = () => callback();
    ipcRenderer.on('shortcut:toggle-find', handler);
    return () => { ipcRenderer.removeListener('shortcut:toggle-find', handler); };
  },
  onEscape: (callback: () => void) => {
    const handler = () => callback();
    ipcRenderer.on('shortcut:escape', handler);
    return () => { ipcRenderer.removeListener('shortcut:escape', handler); };
  },
  onDownloadMedia: (callback: () => void) => {
    const handler = () => callback();
    ipcRenderer.on('shortcut:download-media', handler);
    return () => { ipcRenderer.removeListener('shortcut:download-media', handler); };
  },
